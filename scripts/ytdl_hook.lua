...
local function formats_to_edl(json, formats, use_all_formats)
	...
	for _, track in ipairs(formats) do
		...
		local video_only = track.vcodec and track.vcodec ~= "none" and (not track.acodec or track.acodec == "none")
...
local function add_single_video(json)
	...
	stream_delay = math.max(stream_delay, get_stream_delay(has_requested_formats and requested_formats or { json }))
...
local function on_load_hook(load_fail, hook)
	...
	if stream_delay > 0 then
		msg.info(("Waiting %.2f seconds for the stream to become available..."):format(stream_delay))
		hook:defer()
		mp.add_timeout(stream_delay, function()
			hook:cont()
		end)
	end
end

mp.add_hook("on_load", 10, function(hook)
	on_load_hook(false, hook)
end)
mp.add_hook("on_load_fail", 10, function(hook)
	on_load_hook(true, hook)
end)
